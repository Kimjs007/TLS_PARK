...
{$IFDEF WIN64}
  DllMesCom ,
{$ENDIF}
  Winapi.WinSock, DefCommon, IdFTPCommon, System.Generics.Collections,
...
{$IFDEF WIN64}
    ...
    procedure GetReadData(nType : Integer; sMessage: string);
{$ENDIF}

    procedure SeperateR2RData(sMsg: string);
  ...
  public
    ...
    MesData       : array[0 .. MAX_CH] of TGmesDataPack;
    hMainHandle   : HWND;
    hTestHandle1  : HWND;
    hTestHandle2  : HWND;
    m_sLotNo      : string;
...
{$IFDEF WIN64}
  ...
  FCanUseHost := CommTibRv.Initialize(CommTibRv.TYPE_GMES_64_MES,sServicePort, sNetwork, sDemonPort, sLocal, sRemote);
{$ENDIF}
  m_sLocal  := sLocal;
  m_sRemote := sRemote;
...
begin
  if FMesRtnCd = '0' then begin
    ...
    OnGmsEvent(DefGmes.MES_EAYT,0,	True,'Error code:'+FMesRtnCd+' : '+FMesErrMsgLc + '('+ FMesErrMsgEn + ')');
  end;
end;
...
begin
  ...
  if FMesRtnCd = '0' then begin
    ...
    OnGmsEvent(DefGmes.MES_EDTI,0,True,'Error code:'+FMesRtnCd+' : '+FMesErrMsgLc + '('+ FMesErrMsgEn + ')');
  end;
end;
...
function TGmes.PosEx(const SubStr, Str: string; Offset: Integer): Integer;
var
  I, L1, L2,j: Integer;
  Found: Boolean;
begin
...
{$IFDEF WIN64}
  ...
  m_sR2RLocal:= sLocal;
  FCanUseR2R := CommTibRv.Initialize(CommTibRv.TYPE_GMES_64_R2R,sServicePort, sNetwork, sDemonPort, sLocal, sRemote);
{$ENDIF}
...
procedure TGmes.GetReadData(nType: Integer; sMessage: string);
begin
  case nType of
    CommTibRv.TYPE_GMES_64_MES : GetHostData(sMessage);
    CommTibRv.TYPE_GMES_64_EAS : GetEasData(sMessage);//GetEasData(UTF8ToString(sMessage));
    CommTibRv.TYPE_GMES_64_R2R : GetR2RData(sMessage);//GetR2RData(UTF8ToString(sMessage));
  end;
end;
{$ENDIF}
...
{$IFDEF WIN64}
  ...
  CommTibRv.OnTibRetrunMsg := GetReadData;
{$ENDIF}
...
{$IFDEF WIN64}
  ...
  m_sEasRemote := sRemote;
  m_sEasLocal:= sLocal;
  FCanUseEas := CommTibRv.Initialize(CommTibRv.TYPE_GMES_64_EAS,sServicePort, sNetwork, sDemonPort, sLocal, sRemote);
{$ENDIF}
...
//06:16:38.454 [SEND]
    DefGmes.MES_EICR : begin
      ...
      sSendMsg := sSendMsg  + ' BLID=[]';
      sSendMsg := sSendMsg  + ' JIG_ID=['+ sZigId+ '_' + IntToStr(nPg+1) + ']';
      sSendMsg := sSendMsg  + format(' INSPCHANEL_A=%d',[nPg]);
      sSendMsg := sSendMsg  + ' LOT='+ MesData[FMesPg].LotNo;
      // Added by SHPARK 2023-07-21 오전 11:56:21 GMES 요청사항에 따라 하기와 같이 진행.
      sSendMsg := sSendMsg  + ' PF=P';
      sSendMsg := sSendMsg  + ' RWK_CD=';
      if Trim(MesData[FMesPg].Pf) = '' then begin
        if MesData[FMesPg].Rwk = '' then sSendMsg := sSendMsg  + ' AUTO_INSP_CODE='
        else                             sSendMsg := sSendMsg  + ' AUTO_INSP_CODE=TLS_NG';
      end
      else begin
        if MesData[FMesPg].Pf = 'P' then sSendMsg := sSendMsg  + ' AUTO_INSP_CODE='
        else                             sSendMsg := sSendMsg  + ' AUTO_INSP_CODE=TLS_NG';
      end;
//      // PF가 없는 경우가 있어 방어 코드.
//      if Trim(MesData[FMesPg].Pf) = '' then begin
...
{$IFDEF WIN64}
    ...
    if (nMsgType = DefGmes.R2R_EODS) or (nMsgType = DefGmes.R2R_EODS_R) or(nMsgType = DefGmes.R2R_EODA)   then
      bRtn := CommTibRv.Send_Data(CommTibRv.TYPE_GMES_64_R2R,sSendMsg)
    else if nMsgType <> DefGmes.EAS_APDR then begin
      bRtn := CommTibRv.Send_Data(CommTibRv.TYPE_GMES_64_MES,sSendMsg);
    end
    else begin
      bRtn := CommTibRv.Send_Data(CommTibRv.TYPE_GMES_64_EAS,sSendMsg);
    end;
