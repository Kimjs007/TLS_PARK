...
uses
  ...
  System.Generics.Collections, CtrlDio_Tls, UserUtils, DllLgdCssTls, CommThermometerMulti, CommModbusRtuTempPlate,
  DefCommon, {UdpServerClient,}CommPG, DefScript, uSystemLibrary, IdGlobal, Vcl.Forms, CtrlCssDll_Tls,
  CommonClass, DefPG, CommGmes, DefGmes, {LogicVh,} DefDio, System.Math , DllCasSdkCa410, CommIonizer
...
//    IsNg        : array[0..DefPG.MAX_FRAME_SIZE] of Boolean;
  ...
  TTestInformation = class
    ...
    Model: string;
    ModelConfig: String;
    UseChannel    : array[0..DefCommon.MAX_JIG_CH] of boolean;
    IsPowerOn     : array[0..DefCommon.MAX_JIG_CH] of boolean;
    IsTestDone    : array[0..DefCommon.MAX_JIG_CH] of boolean;
    MainSerialNo  : array[0..DefCommon.MAX_JIG_CH] of string;
    MainPID       : array[0..DefCommon.MAX_JIG_CH] of string;
    DefectCode    : array[0..DefCommon.MAX_JIG_CH] of string;
    ApdrData      : array[0..DefCommon.MAX_JIG_CH] of string;
    Ch            : Integer;
    UserID        : String;
    PowerOn       : Boolean;
    IsScanned     : Boolean;
    ...
    Full_name     : string;
    KeyIn         : string;
    ERR_Code      : array[0..DefCommon.MAX_JIG_CH] of string;
    ERR_Message   : array[0..DefCommon.MAX_JIG_CH] of string;
    MES_Code      : array[0..DefCommon.MAX_JIG_CH] of String;
    NgCode        : array[0..DefCommon.MAX_JIG_CH] of Integer;
    NG_EICR       : Boolean;
    USE_MES       : Boolean;
    USE_GIB       : Boolean;
    Use_ECS       : Boolean;
    ...
    Result        : array[0..DefCommon.MAX_JIG_CH] of string;
    OKCount      : array[0..DefCommon.MAX_JIG_CH] of Integer; //OK 개수
    NGCount      : array[0..DefCommon.MAX_JIG_CH] of Integer; //NG 개수
    PlcRet        : Integer;
    csvHeader     : string;
    csvSubHead1   : string;
    csvSubHead2   : string;
    ...
    StUnitTact    : array[0..DefCommon.MAX_JIG_CH] of   TDateTime;
    EdUnitTact    :  array[0..DefCommon.MAX_JIG_CH] of TDateTime;
...
//    FRxData : TRxData;
    ...
    m_FlashAllData    : TFlashData;
    FIsTempCsvSave: Boolean; //#m_FlashData (CommPG -> Logic:inspector-specific)

    procedure PgToComm_Proc(AMachine:TatVirtualMachine);
    procedure PgCommand_Proc(AMachine:TatVirtualMachine);
    ...
    // TLS Only
    procedure Tls_CurrentStatus_Proc(AMachine:TatVirtualMachine);
    procedure Tls_GetSerialNo_Proc(AMachine:TatVirtualMachine);
    procedure Tls_SetTempPlate(AMachine:TatVirtualMachine);
    procedure Tls_setCurrentChannel_Proc(AMachine:TatVirtualMachine);
    procedure Tls_SetSaveTempCsv_Proc(AMachine:TatVirtualMachine);
    procedure Tls_GetChannelStatus_Proc(AMachine:TatVirtualMachine);
    ...
    procedure OcFlowFinal_Proc(AMachine:TatVirtualMachine);
    procedure OcOnOffCtrl_Proc(AMachine:TatVirtualMachine);
    procedure GetErrCode_Proc(AMachine:TatVirtualMachine);
    ...
    procedure ControlIonizer_Proc(AMachine:TatVirtualMachine);
    procedure ControlLamp_Proc(AMachine:TatVirtualMachine);
    procedure Bcr_Ready_Proc(AMachine:TatVirtualMachine);
...
//    function MIPIWrite(sCommand: String): DWORD;
    ...
    procedure SetIsTempCsvSave(const Value: Boolean);
...
//    procedure SetRxData(const Value: TRxData);
  ...
  public
    m_bUse : Boolean;
    m_bUsed : array[0..DefCommon.MAX_JIG_CH] of Boolean;

    FBcrData, FMesRetrunSerial : array[0..DefCommon.MAX_JIG_CH] of string;
...
//    property TouchData : TRxData read FRxData write SetRxData;
    property IsTempCsvSave : Boolean read FIsTempCsvSave write SetIsTempCsvSave;

// Added by Clint 2023-05-18 오후 9:05:56
    procedure LgdDllCssCb_Ca410Measure(nProbeNo : Integer; var data : TCssCaMeasure);
    procedure LgdDllCssCb_RetInt(nIdx, nParam : Integer; var nRet : Integer);
    procedure LgdDllCssCb_SetInt(nIdx, nParam1, nParam2 : Integer);
    procedure LgdDllCssCb_RetStr(nIdx, nParam : Integer; var sRet : string);
    procedure LgdDllCssCb_Flash_Erase(nType,address, nLen, nTimeOut : Integer; var nRet: Integer);
    procedure LgdDllCssCb_Flash_Read(nType,address, nLen, nTimeOut : Integer;var nRet : Integer; var RetData : TIdBytes);
    procedure LgdDllCssCb_Flash_Write(nType,address, nLen, nTimeOut : Integer;var nRet : Integer; Data : TIdBytes);
    procedure LgdDllCssCb_Result(nIdx, nParam : Integer; sRet : string);
  end;
var
  PasScr : array[defCommon.CH1 .. defCommon.MAX_JIG_CH] of TScrCls;
  PasScrMain : TScrCls;
implementation

{ TScrCls }

constructor TScrCls.Create(nPgNo: Integer; hMain, hTest: HWND; IsMainScript : Boolean );
var
  sEventName: String;
  nCh : Integer;
  i: Integer;
begin
  ...
  m_bTotalTact   := False;
  m_bUnitiTact   := False;
  m_IsMainScript := IsMainScript;
  m_nCurChannel  := 0;
  m_sNgMsg := '';
  FIsTempCsvSave := False;
...
//  if IsMainScript then begin
//    for nCh := 0 to DefCommon.MAX_JIG_CH do  InspectInfo := TTestInformation.Create;
//  end;
  TestInfo := TTestInformation.Create;
  for i := 0 to DefCommon.MAX_CH do begin
    TestInfo.OKCount[i]:= 0;
    TestInfo.NgCode[i] := 0;
  end;
...
//  SetPaScript.AllowDLLCalls:= True; //Use DLL
  ...
  // TLS Only.
  SetPaScript.DefineMethod('f_GetTlsCurrentSatus', 4,tkInteger, nil,Tls_CurrentStatus_Proc,False,0).SetVarArgs([1,2,3]);
  SetPaScript.DefineMethod('f_SetTemperature', 2,tkInteger, nil,Tls_SetTempPlate,False,0);
  SetPaScript.DefineMethod('f_SetCurChannel', 1,tkInteger, nil,Tls_setCurrentChannel_Proc,False,0);
  SetPaScript.DefineMethod('f_SetSaveTempCsv', 2,tkInteger, nil,Tls_SetSaveTempCsv_Proc,False,0);
  SetPaScript.DefineMethod('f_GetTlsBcrData', 3,tkInteger, nil,Tls_GetSerialNo_Proc,False,0).SetVarArgs([1,2]);
  SetPaScript.DefineMethod('f_GetChannelStatus', 4,tkInteger, nil,Tls_GetChannelStatus_Proc,False,0).SetVarArgs([1,2,3]);
  ...
  SetPaScript.DefineMethod('f_OC_VerifyStart',      2,tkInteger, nil,OCVerifyStart_Proc,False,0);
  SetPaScript.DefineMethod('f_ThreadStateCheck',      1,tkInteger, nil,OCThreadStateCheck_Proc,False,0);
  SetPaScript.DefineMethod('f_OcFlowFinal',      1,tkInteger, nil,OcFlowFinal_Proc,False,0);
  SetPaScript.DefineMethod('f_OcOnOffControl',      1,tkInteger, nil,OcOnOffCtrl_Proc,False,0);
  SetPaScript.DefineMethod('f_GetErrorCode',      2,tkInteger, nil,GetErrCode_Proc,False,0).SetVarArgs([1]);
  ...
  SetPaScript.DefineMethod('f_ControlDio', 2,tkInteger, nil,ControlDio_Proc,   False,1);

  SetPaScript.DefineMethod('f_ControlLamp',     1,tkInteger, nil,ControlLamp_Proc,   False);
  SetPaScript.DefineMethod('f_ControlIonizer',  1,tkInteger, nil,ControlIonizer_Proc,   False);
  // for PG.
  SetPaScript.DefineMethod('f_PgCmd',  1,tkInteger, nil,PgCommand_Proc,   False);
  // Bcr Set.
  SetPaScript.DefineMethod('f_Bcr_Ready',  1,tkNone,    nil,Bcr_Ready_Proc,      False,9);
...
// AArgCount : Max Parameter. ADefArgCount : Min Parameter.
  SetPaScript.DefineMethod('f_PgToComm',      5,tkInteger, nil,PgToComm_Proc,      False,5);
...
//  SetPaScript.DefineMethod('f_WriteFlash_Mipi2SRAM', 1,tkInteger, nil,WriteFlash_Mipi2SRAM_Proc,      False);
  ...
  SetPaScript.DefineMethod('f_TactTime',   3,tkNone,    nil,TactTime_Proc,   False,1);
...
//  SetPaScript.DefineMethod('f_EnableVoltSet',  2,tkInteger,  nil, EnableVoltSet_Proc, False);
  ...
  SetPaScript.AddVariable('c_nCurCh',m_nCurChannel);
...
function TScrCls.GetNGCode_ByErroCode(sErrorCode: string): Integer;
var
  nCount: Integer;
  i: Integer;
begin
...
var
  nSlaveType, nWriteData, nRegAddr : Integer;
  sWriteData : string;
  bDataLog : Boolean;
...
begin
  With AMachine do begin
    ...
    nRegAddr   := GetInputArgAsInteger(0);      // arg[0:M] nDevAddr: Integer
    ...
    case wdRet of
      WAIT_OBJECT_0:  sDebug := 'I2C WRITE: TX:DATA: ' + sTxData;
...
//  sPgVer := sPgVer + '/' + Trim(Copy(PG[FPgNo].m_sFwVer,17,4));
  ...
  TestInfo.Ch:= FPgNo;
  m_nCurChannel := FPgNo;
  if m_IsMainScript then begin
    for i := 0 to DefCommon.MAX_JIG_CH do begin
      TestInfo.UseChannel[i] := Self.m_bUsed[i];
      TestInfo.IsTestDone[i]    := False;
    end;
    for i := 0 to DefCommon.MAX_JIG_CH do begin
      if Self.m_bUsed[i] then begin
        m_nCurChannel := i;
        Break;
      end;
    end;
    ...
    for i := 0 to DefCommon.MAX_JIG_CH do begin
      TestInfo.UseChannel[i] := Self.m_bUse;
    end;
    if PasScrMain <> nil then PasScrMain.FBcrData[FPgNo] := '';
  end;
...
procedure TScrCls.InitialMainScript;
begin
  try
...
// Size , Buffer.
procedure TScrCls.LgdDllCssCb_Ca410Measure(nProbeNo : Integer; var data: TCssCaMeasure);
var
  mData : TLvXYUV ;
  nRet : Integer;
  sDebug : string;
begin
  nRet := DllCaSdk2.MeasureData(nProbeNo,mData);
  data.x := mData.x;
  data.y := mData.y;
  data.u := mData.u;
  data.v := mData.v;
  data.deltaXy := mData.dUv;
  data.deltaUv := mData.dXy;
  data.Lv := mData.Lv;
  data.Ret := nRet;
end;

procedure TScrCls.LgdDllCssCb_Flash_Erase(nType, address, nLen, nTimeOut: Integer; var nRet: Integer);
begin

end;

procedure TScrCls.LgdDllCssCb_Flash_Read(nType, address, nLen, nTimeOut: Integer; var nRet: Integer; var RetData: TIdBytes);
var
  ret : integer;
  sDebug : string;
begin
  sDebug := format('[Flash Read] Addr(%0.2x) Len(%d) :',[address,nLen]);
  ScriptLog(sDebug);
  m_FlashAllData.StartAddr := address;
  m_FlashAllData.Size      := nLen;
  m_FlashAllData.bValid    := True;
  ret := Pg[Self.FPgNo].SendFlashRead(address,nLen,@m_FlashAllData.Data[0]);
  CopyMemory(RetData,@m_FlashAllData.Data[0],nLen);
  nRet := ret;
end;

procedure TScrCls.LgdDllCssCb_Flash_Write(nType, address, nLen, nTimeOut: Integer; var nRet: Integer; Data: TIdBytes);
begin

end;

procedure TScrCls.LgdDllCssCb_Result(nIdx, nParam: Integer; sRet: string);
var
  sDebug : string;
begin
  sDebug := format('[Result] nIdx(%d) nParam(%d) :',[nIdx,nParam])+sRet;
  ScriptLog(sDebug);
  case nIdx of
    0 : begin
      Self.TestInfo.DefectCode[self.FPgNo] := sRet;
    end;
    1 : begin
      Self.TestInfo.ApdrData[self.FPgNo] := sRet;
    end;
  end;
...
procedure TScrCls.LgdDllCssCb_RetInt(nIdx, nParam: Integer; var nRet: Integer);
const
  INTERNAL_DEVICE_ADDRESS = $A0;
var
  sCmd, sDebug : string;
  wdRet : DWORD;
  arRData : TIdBytes;
...
//{$IFDEF CA410_USE}
//    DLL_CSS_LGD_IDX_GET_CA410_SYNC     : begin
//      SetCa410Command('STR,1',nRet);
//    end;
//    DLL_CSS_LGD_IDX_GET_CA410_SPEED    : begin
//      SetCa410Command('STR,7',nRet);
//    end;
//    DLL_CSS_LGD_IDX_GET_CA410_MEM_CH   : CaSdk2.GetMemCh(FPgNo,nRet);
//    DLL_CSS_LGD_IDX_SET_CA410_SYNC     : begin
//      sCmd := Format('SCS,%d',[nParam]);
//      SetCa410Command(sCmd,nRet);
//      SetCa410Command('STR,1',nRet);
//    end;
//    DLL_CSS_LGD_IDX_SET_CA410_SPEED    : begin
//      sCmd := Format('FSC,%d',[nParam]);
//      SetCa410Command(sCmd,nRet);
//      SetCa410Command('STR,7',nRet);
//    end;
//    DLL_CSS_LGD_IDX_SET_CA410_MEM_CH   : begin
//      CaSdk2.SetMemCh(FPgNo,nParam);
//      CaSdk2.GetMemCh(FPgNo,nRet);
//    end;
//{$ENDIF}
    DLL_CSS_LGD_IDX_SET_PG_POWER_ON    : begin
//      wdRet := Pg[Self.FPgNo].SendPowerOn(DefScript.PP_COMMAD_PWR_ON_Auto);
//      m_TestRet.PowerOn := True;
//      nRet := wdRet;
      nRet := Pg[FPgNo].SendPowerBistOn(DefPG.CMD_POWER_ON,True,3000,2);
    end;
    DLL_CSS_LGD_IDX_SET_PG_POWER_OFF   : begin
//      m_bFlashAccess := False;
//      wdRet := Pg[Self.FPgNo].SendPowerOn(DefScript.PP_COMMAD_PWR_OFF);
//      m_TestRet.PowerOn := False;
//      nRet := wdRet;
      nRet := Pg[FPgNo].SendPowerBistOn(DefPG.CMD_POWER_OFF,True,3000,2);
    end;
    DLL_CSS_LGD_IDX_GET_TCON_REG   : begin
//      nWait := 1000;
//      wdRet := Pg[Self.FPgNo].SendSpiWp(nParam,nWait);;
//      nRet := wdRet;
      SetLength(arRData,1);
      for i := 0 to 1 do begin //2023-04-07 retry at here (RxParsingErr)
        wdRet := Pg[FPgNo].SendI2CRead(INTERNAL_DEVICE_ADDRESS,nParam,1,arRData, 1000,0{nRetry});
        if wdRet = WAIT_OBJECT_0 then break;
      end;
      nRet := arRData[0];
//      sDebug := format('[TCON Read] Addr(%0.2x) 1 : %0.2x',[nParam,nRet]);
//      ScriptLog(sDebug);

    end;
    DLL_CSS_LGD_IDX_DLL_WORK_DONE : begin
//      self.FCssLgdDllResult := nParam;
//      m_nNgCode := FCssLgdDllResult;
//      RunSeq(DefScript.SEQ_KEY_DLL_FINISH,False);
//
//      nRet := Integer(SecondsBetween(m_TestRet.StartTime,m_TestRet.EndTime));
    end;
  end;
end;

procedure TScrCls.LgdDllCssCb_RetStr(nIdx, nParam: Integer; var sRet: string);
begin

end;

procedure TScrCls.LgdDllCssCb_SetInt(nIdx, nParam1, nParam2: Integer);
const
  INTERNAL_DEVICE_ADDRESS = $A0;
var
  nDataCnt : Integer;
...
begin
  case nIdx of
    DLL_CSS_LGD_IDX_SET_TCON_REG : begin
      nDataCnt := 1;
      SetLength(arRData,nDataCnt);
      arrData[0] := nParam2;
      dwRet := Pg[FPgNo].SendI2CWrite(INTERNAL_DEVICE_ADDRESS,nParam1,nDataCnt, arrData, 200,1);
//      sDebug := format('[TCON Write] Addr(%0.2x) 1 : data - %0.2x',[nParam1,nParam2]);
//      ScriptLog(sDebug);
    end;
  end;
end;
...
procedure TScrCls.OcFlowFinal_Proc(AMachine: TatVirtualMachine);
var
  wdRet, nCh : Integer;
  dTemp : double;
  sData : string;
begin
  With AMachine do begin
    wdRet := 1;
    if InputArgCount = 1 then begin
//      PG[Self.FPgNo].DP860_SendOcOnOff(0{start},2000,0);
//      PG[Self.FPgNo].SetCyclicTimer(True);

      nCh   := GetInputArgAsInteger(0);
      if CtlLgdDll <> nil then CtlLgdDll.FinalWorkDllCss(nCh);
      wdRet := 0;
    end;
...
begin
  With AMachine do begin
    wdRet := 1;
    if InputArgCount = 2 then begin
//      PG[Self.FPgNo].DP860_SendOcOnOff(1{start},2000,0);
//      PG[Self.FPgNo].SetCyclicTimer(False);
      nCh   := GetInputArgAsInteger(0);
      dTemp := GetInputArgAsFloat(1);
      // PID, Serial No, User ID.
      if trim(FMesRetrunSerial[nCh]) = '' then FMesRetrunSerial[nCh] := 'NOSERIALNUM'+ Format('%d',[nCh+1]);
      if trim(FBcrData[nCh]) = '' then FBcrData[nCh] := '1234567890TEST';
      sData := Format('%s,%s,%s,%s',[FBcrData[nCh],FMesRetrunSerial[nCh],Common.m_sUserId,Common.SystemInfo.EQPId]);
      ScriptLog('OC Flow Start : '+sData+', Temp:'+ dTemp.ToString);
      if CtlLgdDll <> nil then CtlLgdDll.StartOcDllCss(nCh,sData,dTemp);
      wdRet := 0;
    end;
    ReturnOutputArg( Integer(wdRet));
  end;
...
begin
  With AMachine do begin
//    wdRet := CSharpDll.MainOC_Stop(Self.FPgNo);

    if CtlLgdDll <> nil then CtlLgdDll.StopDllCss(FPgNo);
    PG[Self.FPgNo].DP860_SendOcOnOff(0{start},2000,0);
    PG[Self.FPgNo].SetCyclicTimer(True);
    wdRet := 0;
    ReturnOutputArg( Integer(0));
  end;
end;

procedure TScrCls.OcOnOffCtrl_Proc(AMachine: TatVirtualMachine);
var
  wdRet, nOn : Integer;
begin
  With AMachine do begin
    ...
    if InputArgCount = 1 then begin
      ...
      PG[Self.FPgNo].DP860_SendOcOnOff(nOn,2000,0);
      PG[Self.FPgNo].SetCyclicTimer(nOn <> 1);
      wdRet := 0;
    end;
    ReturnOutputArg( wdRet);
  end;
...
begin
  With AMachine do begin
    if InputArgCount = 1 then begin
      wdRet := CtlLgdDll.GetOcStatus(Self.FPgNo);
    end;
...
//    PG[Self.FPgNo].SetCyclicTimer(False);
    if InputArgCount = 2 then begin
      nCh   := GetInputArgAsInteger(0);
      dTemp := GetInputArgAsFloat(1);
      if CtlLgdDll <> nil then  CtlLgdDll.StartVerifyDllCss(nCh,dTemp);
      wdRet := 0;
    end;
    ReturnOutputArg( Integer(wdRet));
  end;
...
{$IFDEF CA410_USE}
  ...
  begin
    ...
    begin
      ...
      wdRet := dllCaSdk2.Measure(Self.FPgNo, m_Ca410Data);
...
procedure TScrCls.PgCommand_Proc(AMachine: TatVirtualMachine);
var
  nRet : Integer;
  sCmd : string;
begin
...
//
//procedure TScrCls.PowerMeasure_Proc(AMachine: TatVirtualMachine);
//var
//  pwrData : PReadVoltC;
//  wdRet : Integer;
//  Wrapper: TGenericRecordWrapper;
...
//    if InputArgCount = 1 then begin
//      Wrapper := VarToObject(AMachine.GetInputArg(0)) as TGenericRecordWrapper;
//      pwrData := PReadVoltC(Wrapper.Rec);
//
//      wdRet := Pg[Self.FPgNo].SendPowerMeasure;
//      if wdRet = WAIT_OBJECT_0 then begin
////        SendTestGuiDisplay(defCommon.MSG_MODE_WORKING,'1');
//        CopyMemory(pwrData,@Pg[Self.FPgNo].m_ReadVoltC,SizeOf(Pg[Self.FPgNo].m_ReadVoltC));
//      end;
//      SetInputArg(0,ObjectToVar(Wrapper) );
//      ReturnOutputArg(wdRet);
//    end;
...
//      dwRet:= Pg[FPgNo].SendPowerSet(nSet);
////      dwRet := PGAF9Fpga[FPgNo].AF9_AllPowerOnOff(nSet);
//
//      if dwRet = WAIT_OBJECT_0 then begin
...
begin
{$IFNDEF  SIMENV_NO_PG}
  if not m_IsMainScript then begin
...
*)
  ...
  if SeqStatus[nIdx].Status = ssNone then begin
    case nIdx of
      ...
      DefScript.SEQ_KEY_PWR1_ON  : sSeqName := 'Seq_Ch1_Pwr_On';
      DefScript.SEQ_KEY_PWR2_ON  : sSeqName := 'Seq_Ch2_Pwr_On';
      DefScript.SEQ_KEY_PWR3_ON  : sSeqName := 'Seq_Ch3_Pwr_On';
      DefScript.SEQ_KEY_PWR1_OFF : sSeqName := 'Seq_Ch1_Pwr_Off';
      DefScript.SEQ_KEY_PWR2_OFF : sSeqName := 'Seq_Ch2_Pwr_Off';
      DefScript.SEQ_KEY_PWR3_OFF : sSeqName := 'Seq_Ch3_Pwr_Off';
    end;
    //실행 SEQ 저장
    CurrentSEQ:= nIdx;
  end
...
//    if CurrentSEQ in [SEQ_KEY_START, SEQ_CAM_ZONE, SEQ_UNLOAD_ZONE] then begin
...
begin
  With AMachine do begin
    wdRet := 1;
		Case InputArgCount of
      1 : begin
        //Common.MLog(Self.FPgNo,TestInfo.ApdrData[Self.FPgNo]);
          if DongaGmes <> nil then begin
...
begin
  With AMachine do begin
    wdRet := 1;
		if InputArgCount in [1,2] then begin
      if DongaGmes <> nil then begin
        DongaGmes.MesData[Self.FPgNo].Rwk := TestInfo.MES_Code[FPgNo]; //Common.GmesInfo[nResult].MES_Code;
        //EICR용 Tact 계산
        ...
        Common.MLog(self.FPgNo, 'EICR SKIP - OFF Line');
        wdRet := WAIT_OBJECT_0;
      end;
    end;
    ReturnOutputArg( wdRet);
...
procedure TScrCls.SetIsTempCsvSave(const Value: Boolean);
begin
  FIsTempCsvSave := Value;
end;
...
begin
  With AMachine do begin
    nResult := GetInputArgAsInteger(0); //NgCode
    sMsg := TestInfo.DefectCode[FPgNo];

    if (TestInfo.DefectCode[self.FPgNo] = '0') or (UpperCase(TestInfo.DefectCode[self.FPgNo]) = 'XXXX') then begin
      m_nNgCode := 0;
      nResult   := 0;
    end
    else begin
      m_nNgCode := GetNGCode_ByErroCode(Self.TestInfo.DefectCode[self.FPgNo]);
      nResult   := m_nNgCode;
    end;
    m_InsStatus := isStop;
    TestInfo.NgCode[FPgNo]:= m_nNgCode;
    if m_nNgCode <> 0 then begin
      TestInfo.Result[FPgNo]    :=  Format('%0.2d NG', [nResult]); //Common.GmesInfo[nResult].sErrCode; //Format('PD%0.2d',[nResult]);// 'FAIL';
      TestInfo.ERR_Code[FPgNo]  := Common.GmesInfo[nResult].sErrCode;
      TestInfo.ERR_Message[FPgNo] := Common.GmesInfo[nResult].sErrMsg+ ' ('+m_sNgMsg+')';
      TestInfo.MES_Code[FPgNo]    := Common.GmesInfo[nResult].MES_Code;
    end
    else begin
      TestInfo.Result[FPgNo] := 'PASS';
      TestInfo.ERR_Code[FPgNo]:= '';
      TestInfo.ERR_Message[FPgNo] := '';
      TestInfo.MES_Code[FPgNo] := '';
      sMsg := 'PASS';
      if nSkipResult = 0 then begin
        Inc(TestInfo.OKCount[FPgNo]);
      end;
    end;
    sLog := Format('Result : NgCode(%d),MES_Code(%s),Result(%s),ErrMsg(%s)',[nResult,TestInfo.MES_Code[FPgNo],TestInfo.ERR_Code[FPgNo],TestInfo.ERR_Message[FPgNo]]);
    ScriptLog(sLog);
    TestInfo.IsTestDone[FPgNo] := True;
    SendTestGuiDisplay(defCommon.MSG_MODE_CH_RESULT, sMsg,'', nResult);  // 0: OK, 0 < nResult : NG.
    SendTestGuiDisplay(DefCommon.MSG_MODE_PRODUCT_CNT,'','',TestInfo.OKCount[FPgNo], TestInfo.NGCount[FPgNo]);
  end;
end;
...
var
  nParam, nCh, nIdx : Integer;
begin
  With AMachine do begin
    if InputArgCount = 3 then begin
      nParam := GetInputArgAsInteger(0);
      nCh    := GetInputArgAsInteger(1);
      nIdx   := GetInputArgAsInteger(2);
      case nParam of
        1 : TestInfo.StartTime  := Now;
        2 : TestInfo.EndTime    := Now;
        3 : TestInfo.StUnitTact[nCh] := Now;
        4 : TestInfo.EdUnitTact[nCh] := Now;
      end;
      case nParam of
        1 : SendTestGuiDisplay(DefCommon.MSG_MODE_TACT_START);
        2 : SendTestGuiDisplay(DefCommon.MSG_MODE_TACT_END);
        3 : SendTestGuiDisplay(DefCommon.MSG_MODE_UNIT_TT_START,'','',nCh,nIdx);
        4 : SendTestGuiDisplay(DefCommon.MSG_MODE_UNIT_TT_END,'','',nCh,nIdx);
      end;
    end;
  end;
end;
...
procedure TScrCls.Tls_CurrentStatus_Proc(AMachine: TatVirtualMachine);
var
  nCh : Integer;
  nUse   : Integer;
  wdRet  : DWORD;
begin
  With AMachine do begin
    ...
    if InputArgCount = 4 then begin
      nCh := GetInputArgAsInteger(0);
      if m_bUsed[nCh] then nUse := 1
      else                 nUse := 0;

      SetInputArg(1,nUse);
      SetInputArg(2,g_CommThermometer.CurrentValue[nCh]);
      SetInputArg(3,CommTempPlate.CurrentStatus[nCh+1].Pv);
      wdRet := 0;
    end;
    ReturnOutputArg( wdRet);
  end;
...
procedure TScrCls.Tls_GetChannelStatus_Proc(AMachine: TatVirtualMachine);
var
  nRet, nCh, nUse, nPowerOn, nTestDone : Integer;
...
//      // AS IS
//      if TestInfo.IsPowerOn[nCh] then nPowerOn := 1
//      else                            nPowerOn := 0;
      // To Be.
      if PG[nCh].m_bPowerOn then nPowerOn := 1
      else                       nPowerOn := 0;

      if TestInfo.IsTestDone[nCh] then nTestDone := 1
      else                             nTestDone := 0;

      SetInputArg(1,nUse);
      SetInputArg(2,nPowerOn);
      SetInputArg(3,nTestDone);
      nRet := 0;
    end;
    ReturnOutputArg( nRet);
  end;
...
procedure TScrCls.Tls_GetSerialNo_Proc(AMachine: TatVirtualMachine);
var
  nRet, nCh, nUse : Integer;
...
begin
  ...
  With AMachine do begin
    if InputArgCount = 3 then begin
      ...
      SetInputArg(1,nUse);
      SetInputArg(2,Self.FBcrData[nCh]);
      nRet := 0;
    end;
    ReturnOutputArg( nRet);
  end;
...
procedure TScrCls.Tls_setCurrentChannel_Proc(AMachine: TatVirtualMachine);
var
  wdRet  : DWORD;
begin
  With AMachine do begin
    wdRet := 1 ; // Return NG.
    if InputArgCount = 1 then begin
      Self.FPgNo := GetInputArgAsInteger(0);
      m_nCurChannel := Self.FPgNo;
      wdRet := 0;
    end;
    ReturnOutputArg( wdRet);
  end;
...
procedure TScrCls.Tls_SetSaveTempCsv_Proc(AMachine: TatVirtualMachine);
var
  nValue : Integer;
  sCsvFileName : string;
  wdRet  : DWORD;
...
begin
  With AMachine do begin
    ...
    if InputArgCount = 2 then begin
      ...
      sCsvFileName := GetInputArgAsString(1);
      wdRet := 0;
    end;
    ReturnOutputArg( wdRet);
  end;
...
procedure TScrCls.Tls_SetTempPlate(AMachine: TatVirtualMachine);
var
  nCh, nValue : Integer;
  wdRet  : DWORD;
  dValue : Double;
begin
  With AMachine do begin
    ...
    if InputArgCount = 2 then begin
      nCh := GetInputArgAsInteger(0);
      nValue := GetInputArgAsInteger(1);
      CommTempPlate.SetPlateSV(nCh+1,nValue);
      wdRet := 0;
    end;
    ReturnOutputArg( wdRet);
  end;
  ...
  With AMachine do begin
    wdRet := 1 ; // Return NG.
      ...
      2,3 : begin
        ...
        ControlDio.WriteDioSig(nPos,nVal = 0);
      end;
    End;
    ReturnOutputArg( wdRet);
  end;
...
procedure TScrCls.GetErrCode_Proc(AMachine: TatVirtualMachine);
var
  nCh, nNgCode, nRet : Integer;
begin
  With AMachine do begin
    nRet := 1;
    if InputArgCount = 2 then begin
      nCh :=  GetInputArgAsInteger(0);
      if (TestInfo.DefectCode[nCh] = '0') or (UpperCase(TestInfo.DefectCode[nCh]) = 'XXXX') then begin
        nNgCode := 0;
      end
      else begin
        nNgCode := GetNGCode_ByErroCode(TestInfo.DefectCode[nCh]);
      end;
      SetInputArg(1,nNgCode);
    end;
    ReturnOutputArg(nRet);
  end;
end;
...
begin
  With AMachine do begin
    ...
    nJig := Self.FPgNo div DefCommon.MAX_JIG_CH_CNT;
    SendTestGuiDisplay(DefCommon.MSG_MODE_BARCODE_READY,'','',nBind, 0);
  end;
end;

procedure TScrCls.Bcr_Ready_Proc(AMachine: TatVirtualMachine);
var
  nData, nType : Integer;
...
begin
  with AMachine do begin
    case InputArgCount of
      1 : begin
        nData := GetInputArgAsInteger(0);
        SendTestGuiDisplay(DefCommon.MSG_MODE_BARCODE_READY,'','',nData);
      end;
    end;
  end;
end;
...
const
  INT_PROBE_SET_POSITION_CH1_DN  = 1;
  INT_PROBE_SET_POSITION_CH1_UP  = 2;
  INT_PROBE_SET_POSITION_CH2_DN  = 3;
  INT_PROBE_SET_POSITION_CH2_UP  = 4;
  INT_PROBE_SET_POSITION_CH3_DN  = 5;
  INT_PROBE_SET_POSITION_CH3_UP  = 6;

var
  nMode, nChProbe, nWait     : Integer;
  bIsUp : Boolean;
  wdRet : Integer;
begin           
  With AMachine do begin
    wdRet := 1 ; // Return NG.
      ...
      wdRet := 2;
      case nMode of
        INT_PROBE_SET_POSITION_CH1_DN : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH1,true,nWait);  // up 하면서 이동 후 down.
        INT_PROBE_SET_POSITION_CH1_UP : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH1,false,nWait); // up 상태에서 이동만.
        INT_PROBE_SET_POSITION_CH2_DN : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH2,true,nWait);  // up 하면서 이동 후 down.
        INT_PROBE_SET_POSITION_CH2_UP : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH2,false,nWait); // up 상태에서 이동만.
        INT_PROBE_SET_POSITION_CH3_DN : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH3,true,nWait);  // up 하면서 이동 후 down.
        INT_PROBE_SET_POSITION_CH3_UP : wdRet := ControlDio.MovingProbe(DefDio.PROBE_CH3,false,nWait); // up 상태에서 이동만.
      end;
    end;
    ReturnOutputArg( wdRet);
  end;
...
procedure TScrCls.ControlIonizer_Proc(AMachine: TatVirtualMachine);
var
  nOnOff, i : Integer;
begin
  With AMachine do begin
		if InputArgCount = 1 then begin
      nOnOff   :=  GetInputArgAsInteger(0);

      for i := 0 to Pred(DefCommon.MAX_IONIZER_CNT) do begin
        if Common.SystemInfo.Com_Ionizer[i] = 0 then Continue;
        case nOnOff of
          0 : DaeIonizer[i].SendStop;
          1 : DaeIonizer[i].SendRun;
        end;
         // SendMsg(',RUN,1');
      end;
    end;
...
procedure TScrCls.ControlLamp_Proc(AMachine: TatVirtualMachine);
var
  nOnOff : Integer;
begin
  With AMachine do begin
      ...
      nOnOff   :=  GetInputArgAsInteger(0);
      case nOnOff of
        0 : ControlDio.WriteDioSig(DefDio.OUT_LAMP_WORKER_OFF); // Off.
        1 : ControlDio.WriteDioSig(DefDio.OUT_LAMP_WORKER_OFF,True); // On.
      end;
    end;
  end;
end;
