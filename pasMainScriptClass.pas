...
uses
  ...
  System.Generics.Collections, ControlDio_Tls, UserUtils,
  DefCommon, {UdpServerClient,}CommPG, DefScript, uSystemLibrary, IdGlobal, Vcl.Forms,
  CommonClass, DefPG, CommGmes, DefGmes, {LogicVh,} DefDio, System.Math , Dll_CASDK2
...
//    procedure SetRxData(const Value: TRxData);
  ...
  public
    ...
    InspectInfo : array[0..DefCommon.MAX_JIG_CH] of TTestInformation;
...
//  SetPaScript.AllowDLLCalls:= True; //Use DLL
  ...
  // DLL OC DLOW 관련
  SetPaScript.DefineMethod('f_OcFlowStart',      0,tkInteger, nil,OCFlowStart_Proc,False,0);
...
//  sPgVer := sPgVer + '/' + Trim(Copy(PG[FPgNo].m_sFwVer,17,4));
  ...
  TestInfo.Ch:= FPgNo;
  TestInfo.UseChannel := Self.m_bUse;
...
//      wdRet := CSharpDll.MainOC_Start(Self.FPgNo);
//      wdRet := 0;
//    end
//    else wdRet := 2;
        ...
        case naParam[PARAM_SIGID] of
          ...
          1 : begin // Power
            ...
            //wdRet := Pg[Self.FPgNo].SendPowerOff(nWaitMS,nRetry);
              wdRet := Pg[Self.FPgNo].SendPowerOn(0{Off},nWaitMS,nRetry); //TBD:DP860?
              TestInfo.PowerOn := False;
              ...
              wdRet := Pg[Self.FPgNo].SendPowerOn(1{On},nWaitMS,nRetry); //TBD:DP860?
...
const
  ...
  INT_PROBE_SET_POSITION_CH1_DN  = 3;
  INT_PROBE_SET_POSITION_CH2_UP  = 4;
  INT_PROBE_SET_POSITION_CH2_DN  = 5;
  INT_PROBE_SET_POSITION_CH3_UP  = 6;
  INT_PROBE_SET_POSITION_CH3_DN  = 7;
var
...
begin           
  With AMachine do begin
    wdRet := 1 ; // Return NG.
      ...
      nWait := 100;
      if InputArgCount = 2 then nWait := GetInputArgAsInteger(1);
       nChProbe  :=  GetInputArgAsInteger(1);
      wdRet := 2;
      case nMode of
        0 : wdRet := ControlDio.MovingProbe(nChProbe,true);  // up 하면서 이동 후 down.
        1 : wdRet := ControlDio.MovingProbe(nChProbe,false); // up 상태에서 이동만.
      end;
      wdRet := ControlDio.MovingProbe(nChProbe, bIsUp,nWait);
    end;
    ReturnOutputArg( wdRet);
  end;
