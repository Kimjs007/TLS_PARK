...
interface
  uses
    System.SysUtils, Winapi.Windows, winapi.messages, Vcl.Dialogs, Vcl.Forms, system.Classes, System.Math, DefCommon, IdGlobal, Vcl.ExtCtrls;


const
//  CA_SDK2_DLL = 'CASDK2_DAE.dll';
// Added by Clint 2022-08-05 오전 11:48:39
    CA_SDK2_DLL = '\driver\win32\CASDK2_DAE_32bit_20230127.dll';
    //CA_SDK2_DLL = 'CASDK2_DAE_32bit_20230127.dll';
...
type
  ...
  private
    ...
    FCheckConnection :array [0..pred(DefCaSdk.MAX_CH_CNT)] of Integer;
    ...
    function GetCa410Status(nCh : Integer) : Integer;
...
//    function set_DisplayMode(nCh: Integer) : Integer;
    ...
    // Added by Clint 2022-08-09 오후 4:33:26
    procedure CheckConnection(bEnable : Boolean);   //procedure OnConnectEventTimer(Sender: TObject);
...
//function get_Connect; external CA_SDK2_DLL;
...
begin
  case FSetupCnt.SetupList[nCh].SelectIdx of
    DefCaSdk.CONNECTION_NONE : begin
      ShowMainForm(DefCommon.MSG_MODE_CA410_STATUS,nCh,False,sErrMsg,0) ;
      ...
      if FDeviceCount <> 0 then begin
        ...
        if bIsNg then begin
          ...
          ShowMainForm(DefCommon.MSG_MODE_CA410_STATUS,nCh,True,sErrMsg);
          ShowTestForm(DefCommon.MSG_MODE_WORKING,nCh,True,sErrMsg);
          m_bConnection[nCh] := False;
        end
        else begin
          sErrMsg := '';
          ShowMainForm(DefCommon.MSG_MODE_CA410_STATUS,nCh,False,sErrMsg);
...
function TCA_SDK2.GetCa410Status(nCh: Integer): Integer;
begin
  if not m_bConnection[nCh] then Exit(DefCaSdk.DISCONNECTION_CODE);
  Result := 0;//get_Connect(nCh);
end;
...
procedure TCA_SDK2.CheckConnection(bEnable : Boolean);
var
  nCh : Integer;
begin
  tmrConnectCheck.Enabled := bEnable;
  for nCh := 0 to Pred(defCaSdk.MAX_CH_CNT) do FCheckConnection[nCh] := 0;
end;

procedure TCA_SDK2.OnConnectEventTimer(Sender: TObject);
var
  nCh, nStates : Integer;
  sErrMsg : string;
begin
  for nCh := 0 to Pred(defCaSdk.MAX_CH_CNT) do begin
    if m_bScriptWorking[nCh] then Continue;    // if script is working then skip checking.
    if FCheckConnection[nCh] > 2 then begin
      if m_bConnection[nCh] then begin
        m_bConnection[nCh] := False;
        sErrMsg := Format('Cannel %d CA410 Connection NG',[nCh+1]);
        ShowMainForm(DefCommon.MSG_MODE_CA410_STATUS,nCh,True,sErrMsg);
        ShowTestForm(DefCommon.MSG_MODE_WORKING,nCh,True,sErrMsg);
