...
const
  MAX_PLC_MEMORY_SIZE  = $2100; //8192; //4096; //2048; //255;
...
begin
  ...
  CopyMemory(@AGlassData.PreviousUnitProcessing[0], @naGlassData[38], 4*sizeof(Integer));
  CopyMemory(@AGlassData.GlassProcessingStatus[0], @naGlassData[42], 3*sizeof(Integer));
  CopyMemory(@AGlassData.GlassRoutingData[0], @naGlassData[45], 3*sizeof(Integer));

  AGlassData.PCZTCode:= naGlassData[48];
...
begin
  ...
  CopyMemory(@naGlassData[38], @AGlassData.PreviousUnitProcessing[0], 4*sizeof(Integer));
  CopyMemory(@naGlassData[42], @AGlassData.GlassProcessingStatus[0], 3*sizeof(Integer));
  CopyMemory(@naGlassData[45], @AGlassData.GlassRoutingData[0], 3*sizeof(Integer));

  naGlassData[48]:=AGlassData.PCZTCode;
...
begin
  ...
  SetDeviceDelay('B' + IntToHex(StartAddr_ROBOT+$10*$01+$1 + (nCh * $20), 3), 1, 300); //Glass Data Report
...
begin
  ...
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$01+$0 + (nCh * $20), 3), 0); ////Load Noninterference
  ...
  SetDeviceDelay('B' + IntToHex(StartAddr_ROBOT+$10*$01+$3 + (nCh * $20), 3), 1, 1000); //Load Complete
end;
...
begin
  ...
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$01+$3 + (nCh * $20), 3), 0); //Load Complete off
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$01+$1 + (nCh * $20), 3), 0); //Glass Data Report off
  Sleep(1000);
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$01+$0 + (nCh * $20), 3), 1); //load Noninterference
  //Sleep(300);
  SetDeviceDelay('B' + IntToHex(StartAddr_ROBOT+$10*$01+$2 + (nCh * $20), 3), 0, 300); //Robot Busy off

  if chkAutoStart.Checked and (nCh = 1) then begin
    //자동 시작 체크 되었을 경우
    Sleep(3000);
    SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$01+$D + (nCh * $20), 3), 1); //Inspection Start
  end;
...
begin
  ...
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$00+$1 + (nCh * $20), 3), 0); ////Unload Noninterference
...
//  Sleep(300);
  ...
  SetDeviceDelay('B' + IntToHex(StartAddr_ROBOT+$10*$00+$3 + (nCh * $20), 3), 1, 500); //Unload Complete
...
begin
  ...
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$00+$3 + (nCh * $20), 3), 0); //Unload Complete Off
  Sleep(1000);
  SetDevice('B' + IntToHex(StartAddr_ROBOT+$10*$00+$0 + (nCh * $20), 3), 1); //Unload Noninterference
  //Sleep(300);
  SetDeviceDelay('B' + IntToHex(StartAddr_ROBOT+$10*$00+$2 + (nCh * $20), 3), 0, 300); //Robot Busy
